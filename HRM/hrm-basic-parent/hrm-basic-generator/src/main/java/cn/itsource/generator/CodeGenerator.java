...
public class CodeGenerator {
    ...
    public static void main(String[] args) {
        ...
        ResourceBundle rb = ResourceBundle.getBundle("course-generator");//资源文件名称
        AutoGenerator mpg = new AutoGenerator();

        // 全局配置
        GlobalConfig gc = new GlobalConfig();
        gc.setOutputDir(rb.getString("servicePath") + "/src/main/java");
        gc.setAuthor(rb.getString("author")); //作者名称
        gc.setFileOverride(true);
        gc.setActiveRecord(true);//开启activeRecord模式
        gc.setEnableCache(false);/// xml二级缓存
        gc.setBaseResultMap(true);//xml中生成一个resultmap
        gc.setBaseColumnList(false);//xml公共sql 所有列
        gc.setOpen(false);
